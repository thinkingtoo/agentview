...
class IncrementalScan(unittest.TestCase):
    ...
    def test_only_the_new_lines_are_read_the_second_time(self):
        ...
        self.assertEqual(claude.scan_cached(self.path)["title"], "First")

        reads = []
        original = base.read_from
        base.read_from = lambda p, off: (reads.append(off), original(p, off))[1]
        try:
            write(self.path, [{"type": "ai-title", "aiTitle": "Second"}])
            self.assertEqual(claude.scan_cached(self.path)["title"], "Second")
        finally:
    ...
    def test_an_unchanged_file_is_not_read_at_all(self):
        ...
        claude.scan_cached(self.path)
        calls = []
        original = base.read_from
        base.read_from = lambda p, off: (calls.append(off), original(p, off))[1]
        try:
            self.assertEqual(claude.scan_cached(self.path)["prompt"], "hello")
        finally:
            base.read_from = original
    ...
    def test_a_tool_call_answered_later_stops_being_pending(self):
        ...
        self.assertIsNotNone(claude.scan_cached(self.path)["pending"])
        write(self.path, [{"type": "user", "message": {"content": [
            {"type": "tool_result", "tool_use_id": "t1"}]}}])
        self.assertIsNone(claude.scan_cached(self.path)["pending"])
    ...
    def test_a_truncated_file_is_read_again_from_the_start(self):
        ...
        self.assertEqual(claude.scan_cached(self.path)["title"], "Fresh")
    ...
    def test_half_a_line_at_the_end_is_not_swallowed(self):
        ...
        self.assertEqual(claude.scan_cached(self.path)["title"], "One")
        with open(self.path, "a", encoding="utf-8") as fh:
            fh.write('tle": "Two"}\n')                  # the rest lands
        self.assertEqual(claude.scan_cached(self.path)["title"], "Two")
...
class InFlight(unittest.TestCase):
    ...
    def test_a_call_with_no_result_yet_is_in_flight(self):
        ...
        self.assertAlmostEqual(claude.scan_cached(self.path)["pending"], 25, delta=1)
    ...
    def test_the_newest_call_is_the_one_reported(self):
        ...
        self.assertAlmostEqual(claude.scan_cached(self.path)["pending"], 3, delta=1)
...
class StartingASkill(unittest.TestCase):
    ...
    def test_the_model_calling_the_skill_tool(self):
        self.assertTrue(claude.boss_line({"type": "assistant", "message": {"content": [
    ...
    def test_you_typing_the_slash_command(self):
        ...
        self.assertTrue(claude.boss_line({"type": "user", "message": {"content":
    ...
    def test_another_skill_is_not_the_boss_skill(self):
        self.assertFalse(claude.boss_line({"type": "user", "message": {"content":
            "<command-name>/deploy</command-name>"}}))
        self.assertFalse(claude.boss_line({"type": "assistant", "message": {"content": [
...
class WhatItIsDoing(unittest.TestCase):
    ...
    def test_the_call_in_flight_names_the_work(self):
        ...
        self.assertEqual(claude.scan_cached(self.path)["doing"], "Editing fleet.py")
    ...
    def test_an_answered_call_is_not_what_it_is_doing(self):
        ...
        write(self.path, [{"type": "user", "timestamp": "2026-09-08T10:00:01.000Z",
                           "message": {"content": [
                               {"type": "tool_result", "tool_use_id": "t1"}]}}])
        self.assertEqual(claude.scan_cached(self.path)["doing"], "")
    ...
    def test_a_written_file_is_not_kept_in_memory(self):
        ...
        self.assertEqual(claude.scan_cached(self.path)["doing"], "Writing big.txt")
        state = claude._cache[("state", str(self.path))]
...
class WhatItSaidLast(unittest.TestCase):
    ...
    def test_a_new_prompt_clears_the_last_thing_it_said(self):
        ...
        self.assertEqual(claude.scan_cached(self.path)["said"], "All done.")
        write(self.path, [{"type": "user", "timestamp": "2026-09-08T10:01:00.000Z",
                           "message": {"content": "now do the other thing"}}])
        self.assertEqual(claude.scan_cached(self.path)["said"], "")
    ...
    def test_a_tool_result_is_not_a_new_prompt(self):
        ...
        write(self.path, [{"type": "user", "timestamp": "2026-09-08T10:00:01.000Z",
                           "message": {"content": [
                               {"type": "tool_result", "tool_use_id": "t1"}]}}])
        self.assertEqual(claude.scan_cached(self.path)["said"], "Checking.")
